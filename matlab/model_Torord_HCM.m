...
function output=model_Torord_HCM(t,X,flag_ode, cellType, ICaL_Multiplier, ...
...
% HCM variant of the ToR-ORd model.
...
thLp=3.0*thL;
dhLp=(hLssp-hLp)/thLp;
GNaL=0.0279 * INaL_Multiplier * 2.65; % HCM
if celltype==1
    GNaL=GNaL*0.6;
end
...
diSp=(iss-iSp)/tiSp;
ip=AiF*iFp+AiS*iSp;
Gto=0.16 * Ito_Multiplier * 0.2;  % HCM
if celltype==1
    Gto=Gto*2.0;
elseif celltype==2
    Gto=Gto*2.0;
...
%% The rest
PCa=8.3757e-05 * ICaL_PCaMultiplier * 1.25; % HCM
...
GKs= 0.0011 * IKs_Multiplier * 0.45; % HCM
if celltype==1
    GKs=GKs*1.4;
end
...
GK1=IK1_Multiplier  * 0.6992 * 0.7;  % HCM
if celltype==1
    GK1=GK1*1.2;
elseif celltype==2
    GK1=GK1*1.3;
...
JncxCa=E2*k2-E1*k1;
Gncx= 0.0034* INaCa_Multiplier * 1.5; % HCM
if celltype==1
    Gncx=Gncx*1.1;
elseif celltype==2
    Gncx=Gncx*1.4;
...
Pnak= 15.4509 * INaK_Multiplier * 0.7; % HCM
if celltype==1
    Pnak=Pnak*0.9;
elseif celltype==2
    Pnak=Pnak*0.7;
...
Jrel=Jrel_Multiplier * 1.5378 * 0.7 * ((1.0-fJrelp)*Jrelnp+fJrelp*Jrelp); % HCM
end
